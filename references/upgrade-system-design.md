...
- **전체 프로필 저장**: 초기 설계에서는 `architectureType`, `srcRoot`, `pathAlias` 3개만 저장하려 했으나, SKILL.md의 플레이스홀더가 24개이고 소스가 `devServer`, `scripts`, `tdd`, `git`, `naming`, `layers` 등에 분산되어 있어 전체 프로필을 저장한다. 이렇게 해야 업그레이드 시 재스캔/재문답 없이 managed 파일을 재생성할 수 있다.
