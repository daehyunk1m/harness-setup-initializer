...
- **templateHash**: 버전 번호 대신 해시를 사용한다. 버전별 "어떤 템플릿이 어떤 내용을 생성하는지" 매핑 테이블이 불필요하다. 현재 파일 해시와 manifest 해시를 비교하면 사용자 수정 여부를 즉시 판별할 수 있다.
...
### 2.3 managed 파일의 사용자 수정 대응

managed 파일이라도 사용자가 수정했을 수 있다. 업그레이드 시 `templateHash`와 현재 파일 해시가 다르면 3가지 선택지를 제시한다:

1. **덮어쓰기** — 최신 템플릿으로 교체 (사용자 변경 소실)
2. **스킵** — 현재 파일 유지. manifest에 `"userOverride": true` 표시하여 향후 업그레이드에서도 스킵
3. **병합** — LLM이 마이그레이션의 구조적 변경만 적용하고 사용자 추가분 보존 (best-effort)
...
Phase U2: 계획 제시
  ...
  │ agents/qa-engineer.md        │ 신규 생성│ v4.0 추가 에이전트       │
