...
### 추가 (Added) — Session 35 (2026-06-15)
- scaffold Phase 4 보고 fenced 블록에 `### 이제 할 수 있는 일` 카탈로그(≤12줄, 8능력 + 영속 포인터 1줄). 각 줄 = `[능력] → [트리거] (상세: <정본 경로>)`
- 펜스 직후 "카탈로그 렌더링 규칙" 산문(의미 게이트 앞) — 순수 투영(projection) 명문화: 산출물 게이트 신호(`integrations.multiModelConsult`+§5.16, 생존 `linkedSkills`, 컴패니언 글로벌 전제)를 재사용해 조건부 렌더, 새 게이트 로직·플레이스홀더 0
- §10.2 U5 비대칭 1문장: 업그레이드 보고는 카탈로그 미출력(첫 셋업 전용). NET-NEW 1줄 델타는 OUT-OF-SCOPE
- CLAUDE.md 정합성 검사 항목 5: "카탈로그 각 줄은 산출물을 게이트하는 프로필 신호를 동일하게 가진다 — 미와이어 능력 광고 불가"

### 수정 (Changed) — Session 35 (2026-06-15)
- 기존 "운용 스킬 (선택)" 블록(cleanup/feedback)을 카탈로그가 **흡수** — 중복 제거(재진술 않고 상호참조)
- 의미 정확성 교정: 이슈 #11의 "Security Reviewer = §5.10 게이트" 표기는 거짓(§5.10은 7개 에이전트 항상 생성). 실제 게이트는 session-routine Phase 4.5 SECURITY 호출 조건(`tdd.securityCategories` 매칭) — 카탈로그는 파이프라인 비열거, 정본 지시
- 프로필 스키마 version 1.9.0 → 1.10.0 (두 SKILL.md 동기). README 버전 표기 **1.8.0(1.9.0 미반영 드리프트) → 1.10.0** 정정
- 추적 표면 무변경 명시: harness-check ①·doc-freshness 타깃·manifest files{}·"19개 파일" 카운트·§10.3 마이그레이션 레지스트리 일절 무변경 (신규 생성 파일 0)
...
### 수정됨 (Fixed) — Session 29 (2026-06-13)
- references/upgrade-system-design.md §1.3: 현재 시제 "플레이스홀더 24개" → 26개 (역사 기록이 아닌 시스템 설명이라 갱신)
- references/project-context.md §4 버전 히스토리: 순서 재정렬(1.4.0이 맨 아래·1.6.x 역순으로 뒤섞임 → 오름차순) + **1.6.3 항목 누락 추가**
- SKILL.md §7: "scaffold §5.5/§5.15" → "harness-scaffold/SKILL.md §5.5/§5.15" (교차참조 명확화 — SKILL.md 자체엔 해당 섹션 없음)
- harness-scaffold/SKILL.md §10.3: 마이그레이션 레지스트리에 "M-1.4.0 이후 불필요" 안내 + 등록 판단 기준 명시 (레지스트리가 1.3.0→1.4.0에서 멈춘 것이 정상임을 문서화)
- 프로필 스키마 version "1.6.4" 동기 (계약 변경 없음)

> 감사가 보고한 오탐 3건은 수정하지 않음: 역사 기록의 "24개/12→14/21→24"(그 시점 사실), "15항목"(6.0 디렉토리 준비 제외 시 정확), 마이그레이션 "누락"(1.4.0 이후 의도적 불필요). 감사 에이전트는 "현재 시제 서술 vs 역사 기록" 구분에 약함 — 오탐의 원천으로 기록
