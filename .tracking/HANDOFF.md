...
- **Inc2 잔여 Minor**(`.git/sdd/progress.md` 롤업): 앵커 게이트 grep `([[:space:]]|-->)`가 awk `edge-cases[[:space:]]` 요구와 미묘 불일치 — 공백없는 수작성 앵커에서 false-positive 가능(저빈도). 권고: 게이트를 `[[:space:]]`로 좁혀 정합 + 픽스처 1케이스.
