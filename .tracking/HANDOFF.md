...
1. **신규 셋업 경로 실전 테스트** (TODO-53) — `/harness-setup` → Q&A → `/harness-scaffold` 신규 생성 플로우. 관찰 포인트: 신규 프리셋 매칭(react-vite/express-api + exclude 동작), eslintAssist 옵트인 질문 + ESLint 마커 블록 적용/폴백, feature_list 라우트 추론, harness:check 신규 셋업 판정, **harness-cleanup 첫 실행(CLEANUP_LOG 생성)**
2. **에이전트 템플릿 실전 조정** — TDD subagent 프롬프트 최적화 (실전 사용 피드백 기반)
3. **eslintAssist legacy JS 형식 전략** (TODO-67) — 폴백 발동률 관찰 후 결정
4. **harness-check 검사 항목 확장 검토** (TODO-68) — 체크리스트 §6 엔트로피 항목. harness-cleanup M1(문서-실구조 일치)과 역할 분담 주의: 스크립트=기계 검사, 스킬=판단 검사
