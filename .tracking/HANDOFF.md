...
- **Session 13 (04-08)**: Hook-driven continuation + 디렉토리 재구조화 (TODO-55)
  ...
  - scaffold를 `.claude/skills/harness-scaffold/`로 이동 → 단일 `--add-dir` 등록
...
**.claude/skills/harness-scaffold/SKILL.md (harness-scaffold)** — 스캐폴딩 + 검증 + 보고 전용
...
- **.claude/skills/harness-scaffold/SKILL.md**: `user-invocable: false`, `!command`로 프로필 주입. Stop hook이 매니페스트 생성까지 완료 강제
- 이중 안전장치: Stop hook (`decision: "block"`) + 프롬프트 지시 (§ 4 Step 5)
- 등록: `--add-dir ~/.claude/skills/harness-setup` 하나로 두 스킬 자동 디스커버리
