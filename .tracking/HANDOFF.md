...
> 작성일: 2026-06-10 (갱신 — 1.1.0 하네스 구성 체크리스트 기반 보강)
...
**현재 버전: 1.1.0** (하네스 구성 체크리스트 기반 보강)
...
1. **2-스킬 플로우 실전 테스트** — `/harness-setup` → `.harness-profile.json` → `/harness-scaffold` 전체 플로우 검증 (TODO-53). 1.1.0 신기능 관찰 포인트 포함: harness:check 판정, AGENTS.md 100줄 준수(명령어 섹션 추가 후), eslintAssist 옵트인 플로우, ESLint 마커 블록 실전 검증 (TODO-66)
...
9. **마이그레이션 작성** — managed 파일은 자동 감지로 처리되므로, custom/new/remove 변경 시에만 레지스트리에 추가
