...
**▶ 즉시 다음 — 이슈 #12 증분 2 (TODO-95)**: E2E를 TDD 사이클에 배선한다. ① `session-routine.md` 조건부 **VERIFY(E2E) 단계**(REVIEW/SECURITY 이후, `category:ui` 또는 렌더/이벤트 핸들러 변경 시) ② `coding-standards.md` **`@critical` 태그** 정의 ③ **무의존 pre-push 훅**(@critical 스펙 게이팅) ④ 에이전트 배선(architect.md:60 E2E 슬롯 완성, test-engineer.md:19 E2E 작성 지침, debugger.md 브라우저 재현 모드) ⑤ e2e 전용 eslint override(eslintAssist 켜진 경우, 설계 §6). 설계 정본 §11 증분 2 범위. 증분 1(1.11.0, e2e 옵트인 모듈 — `templates/e2e/*`·§5.17)이 전제. **착수 전 확정할 열린 결정 2건**: (a) E2E 스펙 작성 주체 — 신규 경량 E2E 에이전트(이슈 #12 권장) vs Test Engineer 확장, (b) pre-push 강제 방식 — 무의존 git hook(이슈 #12 권장) vs Husky 도입. → 새 세션에서 brainstorming으로 두 결정부터 정리 후 spec→plan→execute.
