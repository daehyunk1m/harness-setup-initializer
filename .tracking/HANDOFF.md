...
| P7 검증 피드백 루프 | ✅ 완료 | 재스캔/재생성 플로우 추가. 1.11.0: E2E L4 스캐폴드(옵트인) — Playwright 구조 생성, 스위트 통과는 앱별 부팅 의존. 1.12.0: E2E TDD 배선(VERIFY Phase 4.7 + @critical + debugger 재현, 증분 2a). 1.13.0: VERIFY(E2E) 러너 정합(`{{E2E_COMMAND}}`) — 유닛 러너 거짓 PASS 차단(haja 파일럿) |
...
**▶ 즉시 다음 — 이슈 #12 증분 2b (TODO-95b)**: 무의존 pre-push 훅(`.githooks/pre-push` + core.hooksPath, 결정 b)으로 @critical E2E 스펙을 게이팅한다. e2e 전용 eslint override(eslintAssist 시 별도 마커)도 이 증분. 설계 정본 §8(증분 2b 보류 항목). 증분 2a(1.12.0, E2E TDD 배선)가 전제. **착수 전 해소할 9개 난제**(적대적 자문이 식별, TODO-95b 참조): 공존성·PM 비종속·monorepo·@critical 탐지(--list)·실제 설치 판정·별도 eslint 마커·멱등성(외부 git config 상태)·보안 고지·신규파일 마이그레이션. 인프라 변경이라 2a와 실패 도메인이 다름 → 신규 세션에서 spec→plan, 신규 managed 파일이 생기므로 1.14.0(MINOR — 1.13.0은 파일럿 마찰 수정에 소비됨). **참고**: 증분 2의 두 열린 결정은 1.12.0에서 (a) Test Engineer 확장, (b) 무의존 git hook으로 확정됨.

1. **신규 셋업 경로 실전 테스트** (TODO-53) — `/harness-setup` → Q&A → `/harness-scaffold` 신규 생성 플로우. 관찰 포인트: 신규 프리셋 매칭(react-vite/express-api + exclude 동작), eslintAssist 옵트인 질문 + ESLint 마커 블록 적용/폴백, feature_list 라우트 추론, harness:check 신규 셋업 판정, harness-cleanup 첫 실행(CLEANUP_LOG 생성), **superpowers 통합 — 미설치(질문 생략 + 산출물에 superpowers 0건) / 설치 시(감지·옵트인·렌더링·100줄 예산)** (TODO-73 잔여 검증)
