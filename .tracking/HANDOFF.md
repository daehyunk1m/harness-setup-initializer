...
1. **신규 셋업 경로 실전 테스트** — `/harness-setup` → Q&A → `/harness-scaffold` 신규 생성 플로우 검증 (TODO-53). 업그레이드 경로는 TODO-66에서 검증 완료. 남은 관찰 포인트: eslintAssist 옵트인 질문 + ESLint 마커 블록 적용/폴백 (신규 셋업 전용), 업그레이드 멱등성 (TODO-70)
