...
**▶ 즉시 다음 — 이슈 #12 증분 4 (TODO-97)** → 목표 1.17.0. **스코핑 완료 핸드오프 정본: `docs/superpowers/specs/2026-06-16-e2e-incr4-handoff.md`** (새 세션은 이 문서부터 읽을 것 — 4항목 현재 상태·결정·편집 대상·file:line 앵커 종합). 4개 작업: **A** §12.6.1 e2e managed 매핑 정렬(무결정·먼저, SKILL.md:1128-1148에 playwright.config.ts·e2e/tsconfig.json 2행 추가 + 1148 deferral 주석 제거) · **B** 프리셋 e2e 기본값(react-next/router-fsd/vite, express 제외) · **C** U1 재감지(base `e2e.enabled`, harness-scaffold §10.3 1743 앞 prepend) · **D** 사용자 E2E 작성 가이드 문서(권장 `templates/e2e/README.md` managed, e2e.enabled 게이트). **착수 전 사용자 확정 결정**: D1(프리셋 기본값=auto-enable vs pre-seed 권장) · D2(가이드 위치/카테고리) · D3(U1 cascade) · D4(단일 1.17.0 vs 분할). 전 항목 신규 플레이스홀더 0(31 불변)·마이그레이션 불필요 설계 가능.
