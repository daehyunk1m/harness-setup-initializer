...
> 작성일: 2026-06-10 (갱신 2026-06-18 — 1.29.0 후속 docs: 카운트 SSoT 전환·§5.14 동기·drift-guard 방어 + 이슈 #15 종결 + 루트 배치 계약 문서(멀티모델 자문) + README 최신화. 전부 버전 무변경)
...
- **Session 56 (06-18)**: 루트 배치 계약 문서 + README 최신화 (**버전 무변경** — docs/references만). ① **멀티모델 자문**(codex 결함·gemini 대안): "루트 풋프린트 ~30개를 `.harness/`로 정돈할까" 구조 검토 → **전체 이동 보류** 결론(정돈 이득 < 마이그레이션 비용·블래스트 반경; `docs/`·`scripts/`는 생태계 관례로 루트 유지; `agents/`만 후보지만 `.claude/agents/` 이동은 네이티브 디스커버리 시맨틱 변경이라 PoC 선행; pull 수요 0 → 2b-4와 동일 보수성 게이트). ② **루트 배치 계약 문서**(`references/root-placement-contract.md`): 🔒 디스커버리/훅·📁 관례·📦 이동가능(보류) 등급표 + 재검토 트리거 + 강행 시 rename-migration 절차. CLAUDE.md 파일맵·SKILL.md §11·project-context 설계결정 등록. ③ **README 최신화**: `intent-distill` 6스킬 반영(설치·컴패니언 표·디렉토리·하네스란), harness-check 그룹핑 정정(품질 ④⑤⑥·경고 ⑦⑧⑨ → 품질 ④⑤·경고 ⑥⑦⑧⑨⑩, 10항목), intent ledger 산출물 4행, references 목록(root-placement·model-selection 추가), 설계결정 2건(제품 의도 파이프라인·루트 배치 계약). 자문 아티팩트는 `.gitignore` 처리됨(비추적).
