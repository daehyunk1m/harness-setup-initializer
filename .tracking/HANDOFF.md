...
> 작성일: 2026-06-10 (갱신 — 1.2.0 비대화형 검증 명령 보장, 실전 테스트 완료)
...
**현재 버전: 1.3.0** (프리셋 확장 + domain 템플릿 + 추론 정책)
...
| P10 엔트로피 관리 | 📌 범위 밖 | doc-freshness.ts 감지 + 1.1.0 운영 사이클 문서화(CLAUDE.md) + harness:check 자가진단. 주기 실행은 여전히 사용자/별도 cleanup 스킬 영역 |
...
1. **신규 셋업 경로 실전 테스트** (TODO-53) — `/harness-setup` → Q&A → `/harness-scaffold` 신규 생성 플로우. 관찰 포인트: 신규 프리셋 매칭(react-vite/express-api + exclude 동작), eslintAssist 옵트인 질문 + ESLint 마커 블록 적용/폴백, feature_list 라우트 추론, harness:check 신규 셋업 판정
2. **에이전트 템플릿 실전 조정** — TDD subagent 프롬프트 최적화 (실전 사용 피드백 기반)
3. **eslintAssist legacy JS 형식 전략** (TODO-67) — 폴백 발동률 관찰 후 결정
4. **harness-check 검사 항목 확장 검토** (TODO-68) — 체크리스트 §6 엔트로피 항목
5. **Cleanup 스킬 (별도 프로젝트)** — P10 엔트로피 관리 자동화
6. **외부 통합 PRD 구체화** — 멀티모델 합성 자문, superpowers 옵트인 (Session 19 초안)
