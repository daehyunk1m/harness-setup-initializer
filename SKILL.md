...
```
Step 0: 모드 판별
  ├── 하네스 파일 없음 → Setup 모드 (아래 Phase 1~4)
  ├── 하네스 파일 있음 + .harness-manifest.json 없음 → Bootstrap + Upgrade (§ 14.5 → § 14.3)
  └── 하네스 파일 있음 + .harness-manifest.json 있음 → Upgrade 모드 (§ 14.3)
...
| `files.{path}.category` | `managed` / `custom` / `data` (§ 14.2 참조) |
| `files.{path}.templateHash` | 생성 시점 파일 내용의 SHA-256 해시. 사용자 수정 여부 판별 |
| `files.{path}.generatedAt` | 해당 파일의 마지막 생성/갱신 시각 |

#### 생성 규칙

1. **profile 저장**: Phase 1에서 수집한 전체 프로필을 `profile` 필드에 저장한다. 사용자가 "몰라"라고 답해 기본값이 적용된 항목도 포함한다.
2. **해시 계산**: 각 생성 파일의 내용을 SHA-256으로 해싱하여 `files.{path}.templateHash`에 기록한다.
   ...
   ```
3. **카테고리 할당**: § 14.2 파일 카테고리 테이블에 따라 각 파일의 `category`를 설정한다.
4. **files 항목**: 생성한 모든 파일(디렉토리 제외)을 `files`에 기록한다. 키는 프로젝트 루트 기준 상대 경로이다.
5. **version**: 현재 스킬 버전을 `harness.version`과 `harness.skillVersion`에 설정한다.
6. **preset**: 매칭된 프리셋 이름 또는 `"custom"`을 설정한다.
...
4. feature_list.json에 구현할 기능을 추가하세요
5. 첫 번째 기능을 선택하고 TDD 사이클을 시작하세요
...
> **구현 완료**: 프로필 저장 → `.harness-manifest.json` (§ 5.13), 업그레이드 시스템 (§ 14)
...
기존 하네스를 최신 스킬 버전에 맞게 업그레이드한다. `.harness-manifest.json`으로 버전을 추적하고, 마이그레이션 레지스트리에 따라 파일별로 적절한 업그레이드 작업을 수행한다.

### 14.1 매니페스트 상세

매니페스트 스키마와 생성 규칙은 § 5.13을 참조한다. 여기서는 업그레이드에 관련된 추가 사항을 기술한다.
...
| **해시 기반 변경 감지** | 버전별 "어떤 템플릿이 어떤 내용을 생성하는지" 매핑 테이블이 불필요하다. 현재 파일 해시와 manifest 해시를 비교하면 사용자 수정 여부를 즉시 판별할 수 있다 |
...
### 14.3 업그레이드 Phase U1~U5

```
Phase U1: 분석
  ├── .harness-manifest.json 읽기
  ├── 버전 갭 계산: manifest.harness.version → 현재 스킬 버전
  ├── 적용할 마이그레이션 목록 로드 (§ 14.4 레지스트리에서 순서대로)
  └── 파일별 상태 판별:
        ├── data → 스킵
        ├── managed + 해시 일치 → 덮어쓰기 예정
        ├── managed + 해시 불일치 → 사용자 수정 감지, 선택 필요
...
Phase U2: 계획 제시
  ...
  │ 파일                         │ 작업     │ 이유             │
  ├──────────────────────────────┼──────────┼──────────────────┤
  │ agents/architect.md          │ 덮어쓰기 │ managed, 템플릿 업데이트 │
  │ CLAUDE.md                    │ 부분 수정│ custom, 새 섹션 추가     │
  │ agents/qa-engineer.md        │ 신규 생성│ 추가 에이전트            │
  │ feature_list.json            │ 스킵     │ data                     │
  │ agents/reviewer.md ⚠️        │ 사용자 선택│ managed지만 수정 감지   │
  └──────────────────────────────┴──────────┴──────────────────┘
  → 사용자 승인 대기
         ↓
Phase U3: 실행
