...
[Upgrade 모드]
Phase U1~U2 (§ 14.3 참조) → .harness-profile.json 저장 → /harness-scaffold 안내
```
...
사용자가 승인하면 `.harness-profile.json`을 프로젝트 루트에 저장하고, 다음을 출력한다:
...
```json
{
  ...
  "fromVersion": "3.3",
  "toVersion": "4.0",
  "migrations": ["M-3.3-to-4.0"],
  "fileActions": {
    "agents/architect.md": { "action": "overwrite", "reason": "managed, 템플릿 업데이트" },
    "CLAUDE.md": { "action": "surgical", "changes": [] },
    "feature_list.json": { "action": "skip", "reason": "data" }
  }
}
```
...
매니페스트 스키마는 SKILL-SCAFFOLD.md § 5.13을 참조한다. 여기서는 업그레이드에 관련된 추가 사항을 기술한다.
...
마이그레이션 레지스트리, 파일 카테고리, 실행 상세는 SKILL-SCAFFOLD.md § 10을 참조한다.
