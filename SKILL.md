...
> **사용자 실행 방법**: 스킬 디렉토리를 등록하면 두 스킬이 자동으로 로딩된다.
> ```bash
> claude --add-dir ~/.claude/skills/harness-setup
> ```
> `/harness-scaffold`는 `.claude/skills/` 하위에 위치하여 자동 디스커버리된다.
...
매니페스트 스키마는 harness-scaffold/SKILL.md § 5.13을 참조한다. 여기서는 업그레이드에 관련된 추가 사항을 기술한다.
