...
연계 가능한 외부 스킬 패키지의 설치 여부를 확인한다. **미감지 시 관련 질문을 생략한다** (사용자가 모르는 도구를 노출하지 않음).

**superpowers** (현재 유일한 통합 대상 — 매핑 정본: `references/integrations/superpowers-mapping.md`):
```bash
...
- 발견 시 § 4.2의 superpowers 연계 질문(옵트인)을 트리거한다
...
- 동의하면 프로필에 `integrations.superpowers`를 기록하고 (linkedSkills 기본 3종), 거부하면 `integrations` 필드를 생략한다 (연계 없음)
...
- 외부 통합 (superpowers): 옵트인 여부, source/version/installPath, linkedSkills
```
...
| 외부 통합 (superpowers) | 미연계 (`integrations` 필드 생략) | 외부 의존 연계는 명시적 동의가 필요. 미감지 시 질문 자체를 생략 |
...
| `integrations` (선택) | Step 1.6 감지 + Step 4 옵트인 | 생략 시 외부 연계 없음. 현재 지원: `superpowers` — `source`는 `"plugin"`/`"skill-dir"`, `detectedVersion`은 정보용(plugin만), `installPath`는 scaffold의 스킬 실존 검증용, `linkedSkills`는 매핑 정본의 연계 목록에서만 렌더링됨. 프리셋 비대상 |
...
Phase U1: 분석
  ...
  ├── 외부 통합 재감지 (Step 1.6과 동일):
  │     - 새로 감지 + manifest.profile에 integrations 없음 → U2에서 통합 추가 제안 (옵트인)
  │     - 기존 통합 있음 → linkedSkills 실존 재검증, 사용자가 제거를 요청하면 fileActions로 섹션 제거
