...
7. agents/*.md (7개 subagent 정의 — session-routine.md가 참조)
8. feature_list.json (빈 배열 또는 기존 코드 기반 추론)
9. claude-progress.txt (초기 상태 + TDD STATE 블록 포맷)
10. init.sh (스택에 맞는 초기화 스크립트)
11. scripts/structural-test.ts (아키텍처 규칙 검증)
12. scripts/doc-freshness.ts (문서 최신성 검사)
13. docs/QUALITY_SCORE.md (품질 점수표 초기값)
14. docs/TECH_DEBT.md (기술 부채 빈 템플릿)
...
3. feature_list.json → passes: false 중 최고 우선순위 선택
4. npm run validate (회귀 체크)
5. TDD 사이클 시작 (상세: .claude/rules/session-routine.md)
...
# 6.5 .claude/rules/ 확인
ls -la .claude/rules/session-routine.md .claude/rules/coding-standards.md
...
**세션 시작**: claude-progress.txt → git log → feature_list.json → validate (회귀 체크) → TDD 사이클
**세션 종료**: validate → feature_list.json 업데이트 → progress 기록 → git commit
```
