...
**cursor 이후(§1.1) 이벤트만** 분석한다. 단, `infra-track-entry`(감사 마커)·`session-incomplete`(세션 종료 루틴 기록)는 마찰이 아니므로 패턴 분석·보고 기준에서 **제외**한다 — session-routine 세션 종료 트리거의 SKIP 집합과 동일하며(트리거↔보고 정합), 루틴/감사 마커가 "동일 이벤트 ≥2 반복 패턴"으로 오보고되는 것을 막는다. 나머지 이벤트를 분석하여 보고 대상 패턴을 식별한다:
