...
- 프로필 데이터를 기반으로 19개 파일을 의존 순서대로 생성한다 (+ package.json scripts 수정, 옵트인 시 ESLint 설정 수정·superpowers 연계 렌더링)
...
- `integrations`는 사용자가 문답에서 옵트인한 경우에만 존재한다 (선택 필드). 생략 시 외부 연계 산출물을 생성하지 않는다 (§ 5.16). `linkedSkills`는 매핑 정본(`references/integrations/superpowers-mapping.md`)의 연계/선택 목록에 있는 것만 렌더링된다.
...
- **조건부 — 보조 스킬 섹션**: 프로필에 `integrations.superpowers.enabled == true`가 있으면 "문서 맵" 앞에 "## 보조 스킬 (superpowers 연계)" 섹션(약 8줄)을 삽입한다 — 렌더링 규칙: § 5.16. 없으면 섹션 자체를 생성하지 않는다
...
| `{{INTEGRATION_NOTES}}` | 프로필 integrations — superpowers.linkedSkills에 `writing-plans` 포함 시 매핑 정본의 연계 1줄, 그 외/생략 시 **빈 문자열** | 빈 문자열 |
...
### 5.16 superpowers 연계 렌더링 (옵트인)

프로필에 `integrations.superpowers` 필드가 있고 `enabled: true`일 때만 실행한다. 필드가 없으면 연계 산출물을 일절 생성하지 않는다 (AGENTS.md 섹션 없음, `{{INTEGRATION_NOTES}}`는 빈 문자열).

연계/제외 분류 정본: `references/integrations/superpowers-mapping.md` — 이 파일을 읽어 렌더링한다.

#### 렌더링 절차

1. **실존 검증** (F1.7): `linkedSkills`의 각 스킬에 대해 `{installPath}/skills/{스킬명}/` 디렉토리 존재를 확인한다
   - 없는 스킬은 **드롭**하고 Phase 4 보고에 경고한다 (superpowers 버전 변경으로 스킬 이름이 바뀐 경우 등)
   - installPath 자체가 없으면(플러그인 제거됨) 연계 전체를 건너뛰고 보고에 안내한다
2. **제외 필터** (F1.6): 매핑 정본의 "연계"/"선택" 목록에 없는 스킬은 linkedSkills에 있어도 **렌더링하지 않는다** (제외 목록의 TDD·코드 리뷰 등 코어 충돌 스킬 차단)
3. **AGENTS.md 렌더링**: 생존한 linkedSkills로 "## 보조 스킬 (superpowers 연계)" 섹션을 구성한다 (§ 5.1 위치 규칙):

```markdown
## 보조 스킬 (superpowers 연계)

{매핑 정본의 각 연계 스킬 문구 — 1줄씩}

> superpowers 플러그인(v{detectedVersion}) 연계 — 미설치 환경에서는 무시된다.
> TDD·코드 리뷰·검증은 본 하네스 자체 워크플로를 사용한다.
```

4. **session-routine 렌더링**: `writing-plans`가 생존 목록에 있으면 `{{INTEGRATION_NOTES}}`를 매핑 정본의 연계 1줄로, 아니면 빈 문자열로 치환한다 (§ 5.11.3)

#### 원칙

- superpowers 스킬 본문을 복제하지 않는다 — 이름과 호출 시점 안내만
- `detectedVersion`은 정보용 표기일 뿐, 버전 비교/차단을 하지 않는다 (드리프트는 실존 검증이 잡는다)
- 옵트아웃(필드 생략) 시 superpowers라는 단어가 어떤 산출물에도 나타나지 않는다
...
# 6.15 superpowers 연계 검증 (integrations 옵트인 시에만 실행)
grep -q "## 보조 스킬 (superpowers 연계)" AGENTS.md && echo "✅ AGENTS.md 보조 스킬 섹션 존재" || echo "❌ 보조 스킬 섹션 누락"
# 옵트아웃/미감지 시에는 반대로 — AGENTS.md와 .claude/rules/에 "superpowers" 문자열이 없어야 한다
```
...
> **이 레지스트리가 M-1.3.0-to-1.4.0에서 멈춘 것은 정상이다** — 누락이 아니다. 1.4.0 이후 버전은 생성 하네스에 마이그레이션 작업이 필요 없었다:
...
> - **1.6.1~1.6.3**: install.sh·run-advisor.js·Stop hook·문서 수정 — 모두 스킬 인프라이고 생성 하네스 산출물에 영향 없음
>
