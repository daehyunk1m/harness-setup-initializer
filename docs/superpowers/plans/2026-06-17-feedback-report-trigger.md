...
**상태 계약 (모든 태스크 공통):** `.harness-feedback-cursor` = 단일 JSON 라인 `{"processedLines": N, "lastReportedAt": "<ISO|null>"}`. `processedLines` = `wc -l .harness-friction.jsonl`(물리 줄 수 = 이벤트 수, append-only이므로 안전). cursor 부재 = `processedLines:0`(전체 미보고).
