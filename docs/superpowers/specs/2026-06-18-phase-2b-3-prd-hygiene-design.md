...
- **feature_list 비어있음/없음 → invalid-feature·file-marker-mismatch 보류** (ℹ️ 1줄 안내) — `[]` 폴백 프로젝트에서 작성 PRD가 모두 invalid로 오탐되는 폭탄 방지. unbound/multiple/duplicate는 계속 검출(id 집합 불필요).
...
- **골든 픽스처** `test/prd-marker-hygiene-fixtures.sh` (기존 `test/prd-substrate-fixtures.sh` 패턴 미러):
  ...
  - T12 render-after 와이어링 — 플레이스홀더 no-op 치환 후 전체 harness-check.sh 실행 → exit 0 + ⑩ 출력 + 정상 판정
  - T13 빈 feature_list(`[]`) + 유효형 마커 → invalid-feature·mismatch 보류(오탐 방지)
  - T14 빈 feature_list(`[]`) + 마커 없는 PRD → unbound는 계속 검출
