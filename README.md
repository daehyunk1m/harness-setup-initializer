...
> 현재 버전: **1.29.0** · 상세 이력: [`.tracking/CHANGELOG.md`](.tracking/CHANGELOG.md)
...
그리고 운영 단계에서는 **컴패니언 스킬**이 정리·피드백·교차 자문·의도 증류를 돕는다 (아래 [컴패니언 스킬](#컴패니언-스킬) 참조).
...
| | `scripts/harness-check.sh` | 하네스 자가진단 (검사 10항목 — 구조 ①②③·품질 ④⑤·경고 ⑥⑦⑧⑨⑩; ⑧ E2E·⑨ pre-push·⑩ PRD 위생은 해당 조건 존재 시에만, `node_modules` 부재 시 ④⑤⑥ "의존성 미설치로 보류"·exit 0, `npm run harness:check`) |
...
6개 스킬(harness-setup·harness-scaffold·harness-cleanup·harness-feedback·multi-model-consult·intent-distill)이 함께 번들로 로딩된다.
