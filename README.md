<!-- BANNER: 1280×320 권장. 추후 assets/banner.png 추가 후 아래 주석 해제
<p align="center">
  <img src="assets/banner.png" alt="harness-setup" width="100%">
</p>
-->

<h1 align="center">harness-setup</h1>
<p align="center">Node.js/TypeScript 프로젝트에 에이전트 작업 환경(하네스)을 자동 셋업하는 Claude Code 스킬</p>
...
> 상세 변경 이력: [`.tracking/CHANGELOG.md`](.tracking/CHANGELOG.md)

---

소스 코드를 분석하고 사용자와 문답을 거쳐, 프로젝트에 맞는 문서·설정·검증 스크립트를 생성한다. 하네스가 없으면 에이전트는 매 세션 프로젝트를 처음부터 파악해야 하지만, 하네스가 있으면 맥락·규칙·검증 루프를 즉시 갖춘 상태로 작업을 시작한다. 기존 소스 코드는 수정하지 않는다(옵트인한 설정 보강만 예외 — 아래 [설계 결정](#기존-코드-무수정-원칙) 참조).

---

## 빠른 시작

1. Claude Code에서 플러그인을 설치한다:
...
2. 대상 프로젝트에서 자연어나 슬래시 커맨드로 실행한다:
...
질문 2~4개에 답하고 프로필을 승인하면, 나머지(파일 생성·검증·보고)는 자동으로 진행된다. 사용자가 하는 일은 **(1) 셋업 요청, (2) 질문에 답변, (3) 프로필 승인** 세 가지뿐이다.

> 설치 변형·번들 구성은 아래 [설치 상세 & 디렉토리 구조](#설치-상세--디렉토리-구조)를 참고한다.

---

## 하네스란?

에이전트가 프로젝트를 **이해하고, 작업하고, 검증하고, 정리**할 수 있도록 돕는 작업 환경 전체다.
...
- 제품 의도(intended/버그)가 `.harness-intent.jsonl`에 적재되고, intent-distill이 PRD·E2E 커버리지 갭으로 증류 (`docs/INTENT_BACKLOG.md`)
- 인프라/설정 작업은 인프라/설정 트랙으로 유닛 TDD 대신 통합 검증(빌드+실동작)을 적용

운영 단계에서는 **컴패니언 스킬**이 정리·피드백·교차 자문·의도 증류를 돕는다 (아래 [컴패니언 스킬 & 외부 통합](#컴패니언-스킬--외부-통합) 참조).
...
| **`/harness-scaffold`** | Phase 2~4: 파일 생성 + 검증 + 보고 | 하네스 파일 일습 — 문서·`.claude/rules/`·TDD 에이전트·스크립트·데이터 싱크·`.harness-manifest.json` (전체 목록·카테고리: scaffold § 10.1; 옵트인 시 E2E·pre-push 추가) |
...
```
사용자: "하네스 셋업해줘"
→ 자동 스캔 → 2~4개 질문 → 프로필 승인 → 하네스 파일 자동 생성 → 완료
```
...
scaffold가 프로필을 읽어 아래 파일을 의존 순서대로 생성한다. 전체 목록과 업그레이드 카테고리는 `harness-scaffold/SKILL.md` § 10.1이 정본이다.
...
| | `scripts/harness-check.sh` | 하네스 자가진단 — 검사 10항목 (`npm run harness:check`, 아래 참고) |
...
| | `.harness-feedback-cursor` | 마찰 보고 위치 북마크 (harness-feedback이 미보고 마찰만 평가) |
| **의도** | `docs/INTENT_LEDGER.md` | 제품 의도 원장 스키마·유형 정적 참조표 |
| | `docs/INTENT_BACKLOG.md` | 의도 커버리지 2차원 백로그 (PRD·E2E, intent-distill 동기화) |
| | `.harness-intent.jsonl` | 제품 의도 자동 적재 싱크 — append-only JSONL, 빈 파일 (always-on) |
| **기타** | `docs/product-specs/{README,_template}.md` 외 3개 디렉토리 | PRD substrate(관례·양식 managed) + 설계 결정·실행 계획·참고 자료 디렉토리 |
| | `package.json` | `lint:arch`, `validate`, `doc:check`, `harness:check` 스크립트 추가 |
| | `.harness-manifest.json` | 버전 추적 매니페스트 |

`harness:check`는 검사를 세 묶음으로 나눠 판정한다 — 구조 ①②③ / 품질 ④⑤ / 경고 ⑥⑦⑧⑨⑩. ⑧ E2E·⑨ pre-push·⑩ PRD 위생은 해당 조건이 존재할 때만 실질 검사하고, `node_modules` 부재 시 ④⑤⑥을 "의존성 미설치로 보류"로 표기하며 exit 0을 반환한다. "표준 하네스 가동" 판정은 구조 설치·실행 가능성을 확인하는 것이며, 문서·규칙의 의미 정확성은 별도 검토를 권장한다.
...
`presets/` 폴더에 JSON 파일을 만들면 자동으로 매칭 대상에 포함된다. 필수 필드: `name`, `displayName`, `detection.required`, `architecture.type`, `architecture.layers`, `scripts.lint:arch`, `devServer`, `pathAlias`, `srcRoot`. 기존 프리셋을 복사해 수정하는 것이 가장 빠르다.
...
harness-setup 플러그인에 아래 스킬이 번들로 포함되어, 설치하면 자연어로 바로 호출할 수 있다.
...
| **intent-distill** | "의도 정리" / "커버리지 분석" | `.harness-intent.jsonl` 제품 의도를 `@feature` PRD·E2E 실구조와 대조해 2차원 커버리지 갭을 `docs/INTENT_BACKLOG.md`로 동기화 (증거 필수, gh 이슈는 항목별 옵트인) |
| **multi-model-consult** | "멀티모델 자문" / "교차 자문" | Codex·Gemini CLI에 관점을 분담해 자문하고 Claude가 합성(합의/상충/최종방향/액션). 읽기 전용, 하네스 비의존 범용 도구 |

### 외부 통합 (옵트인)

셋업 시 외부 보조 스킬이 감지되면 연계 여부를 묻는다 (미감지 시 질문 자체를 생략). 동의하면 AGENTS.md "보조 스킬" 섹션에 호출 안내가 1줄씩 추가된다. 메커니즘 규약: [`references/integrations/_protocol.md`](skills/harness-setup/references/integrations/_protocol.md).
...
6개 스킬(harness-setup·harness-scaffold·harness-cleanup·harness-feedback·multi-model-consult·intent-distill)이 함께 번들로 로딩된다. 트리거는 자연어 또는 슬래시 커맨드 모두 가능하다 (`하네스 셋업해줘` / `/harness-setup`).

<details>
<summary>레거시(git clone + install.sh)에서 이전하는 경우</summary>

1.23.0부터 배포가 플러그인으로 전환됐다. 기존 클론에서 `./install.sh`를 한 번 실행하면 구 심볼릭 링크를 정리하고 플러그인 설치를 안내한다. `git pull`로는 더 이상 스킬이 갱신되지 않으며, 개발 클론은 `~/.claude/skills/` 밖에 두는 것을 권장한다.
...
│   │       ├── root-placement-contract.md  # 루트 배치 계약 (고정/이동 가능 산출물)
│   │       ├── model-selection-guide.md    # Opus/Sonnet 모델 선택 가이드
...
│   │   ├── templates/                  # 생성 템플릿 (agents·rules·e2e·product-specs·structural-test·scripts·docs)
...
│   ├── intent-distill/SKILL.md         # 의도 원장 → PRD·E2E 2차원 커버리지 증류
│   └── multi-model-consult/
...
<details>
<summary>주요 설계 결정 펼치기 (12개 항목)</summary>
...
### 제품 의도 파이프라인 (1.24.0~1.29.0, 이슈 #15)
대화 중 사용자가 말로 주는 제품 의도(intended/버그)를 `.harness-intent.jsonl`에 always-on 적재하고(마찰 채널의 자매 채널), intent-distill이 `@feature` PRD·E2E 실구조와 대조해 2차원 커버리지 갭을 `docs/INTENT_BACKLOG.md`로 동기화한다. PRD substrate(`docs/product-specs/`)와 정적 위생 검사(harness-check ⑩ — 마커 + 빈 Edge Cases)가 받친다. `encoded`는 비권위이고 커버리지는 매 실행 실구조에서 derive한다(증거 필수). 역방향 "미검증 명세"·교차 derive는 보수성 게이트(입증 못하면 뺀다)로 이연했다.

### 루트 배치 계약 (2026-06-18)
대상 프로젝트 루트에 생성되는 ~30개 산출물 중 무엇이 루트 고정이고(도구 디스커버리·hook 경로) 무엇이 이동 가능한지를 [`references/root-placement-contract.md`](skills/harness-setup/references/root-placement-contract.md)에 박았다. 멀티모델 자문(codex·gemini) 결론: 전체 `.harness/` 이동은 정돈 이득 < 마이그레이션 비용·블래스트 반경이고 pull 수요도 없어 보류. `docs/`·`scripts/`는 생태계 관례로 루트 유지, `agents/`만 후보지만 `.claude/agents/` 이동은 네이티브 디스커버리 시맨틱 변경이라 PoC를 선행한다.
