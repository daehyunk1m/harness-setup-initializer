...
제품 의도(`.harness-intent.jsonl`, Phase 1 수집)를 읽어 각 의도가 `@feature` PRD·E2E로 커버되는지 **실구조에서 파생**하고, 미커버 갭을 `docs/INTENT_BACKLOG.md`(영속 백로그)에 **머지-싱크**한다. harness-feedback의 자매이나, 마찰의 이벤트-스트림 모델이 아니라 **지속 백로그 모델**이다 — 상태 파일(cursor) 없음, 백로그 문서가 durable 상태, 커버리지는 매 실행 derive(저장 flag 신뢰 안 함).
...
- 커버리지(PRD·E2E 2차원)는 매 실행 실구조에서 파생한다 — substrate 부재는 blocked로 미커버와 구분. **증거 필수.**
