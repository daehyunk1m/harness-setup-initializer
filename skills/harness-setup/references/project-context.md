...
| 카운트 표기 SSoT 참조 (1.29.0 후속, 버전 무변경) | 현재시제 "파일 N개"·"검사 N항목" 같은 **깨지기 쉬운 정수를 산문에 하드코딩하지 않고** §10.1 인벤토리·§6 검증·checklist §8을 정본으로 참조하게 전환. 절단-취약 drift-guard(`test/prd-section-body-drift.sh`)는 regex 협소화가 불가할 때 **종단 토큰(`print l`) 존재 단언**으로 false-sync를 방어. | docs-hygiene 정정 중 핸드오프 목표값 "24"도 stale 발견 — 파일이 §10.1에 서브번호(`22-b…22-h`)로 삽입되며 헤드라인 정수가 미추종(실제 ≈30). 정수 하드코딩은 **구조적으로 재발하는 드리프트 클래스** → 단일 정본 참조로 전환(핵심원칙 #5: 반복 문제는 구조로 승격). 사용자 결정("SSoT 참조 전환"). 동작·계약·스키마 불변이라 버전 무변경. |
| 루트 배치 계약 (2026-06-18, 버전 무변경) | 생성 산출물의 루트 고정(🔒 디스커버리/훅·📁 관례)·이동 가능(📦 보류) 등급을 `references/root-placement-contract.md`에 명문화. 전체 `.harness/` 이동은 **보류** — `docs/`·`scripts/`는 관례로 루트 유지, `agents/`만 후보지만 `.claude/agents/` 이동은 네이티브 디스커버리 시맨틱 변경이라 PoC 선행. | 멀티모델 자문(codex 결함·gemini 대안) 결론: 전체 이동은 정돈 이득 < 마이그레이션 비용·블래스트 반경(매니페스트 경로·§12.6 매핑·골든 픽스처·rename-migration), pull 수요 없음(루트 클러터 마찰 기록 0) → 2b-4와 같은 보수성 게이트로 이연. base-dir 변수 주입은 결정론(grep·픽스처) 손상으로 지양. 참조 문서만 추가 → 버전 무변경(references는 Public API 아님). |
...
### 1.21.0 (E2E 아티팩트 .gitignore 머지 — 이슈 #13)
- **1.21.0** (2026-06-17) — E2E 스캐폴드가 `playwright.config.ts`는 생성하면서 실행 산출물(`test-results/`·`playwright-report/`) `.gitignore` 처리를 누락해 스모크 실행 후 untracked로 남던 마찰(이슈 #13, haja-web-fe 파일럿에서 수동 추가 — #12 "빌드 설정 펜스(gitignore)" 의도의 구현 누락분)을 마감. **해결**: harness-scaffold §5.17에 `.gitignore` **외과적 add-only 머지** 추가 — `/test-results/`·`/playwright-report/`(Playwright 공식 루트앵커 컨벤션)를 마커 블록(`harness-setup:e2e-artifacts:start~end`)으로 주입. **멱등·비침습**: 마커 존재 시 스킵, 파일 부재 시 마커 블록만으로 생성, 기존 항목 재정렬·삭제 없음. **중복 회피**: 사용자가 마커 밖에서 이미 무시 중인 항목은 추가하지 않고(토큰 매칭), 둘 다 무시 중이면 no-op — 파일럿처럼 수동 추가한 하네스와 충돌 회피. 텍스트로만 읽고 씀(비실행 원칙). **카테고리 결정**: `.gitignore`는 사용자 소유·마커 블록만 추가 → manifest files 미기록(§10.1 #33, §5.15 ESLint·§5.5 package.json과 동급), §12.6 자동 감지 비대상. **소급 설계**: §10.2 U3 step 6-b — add-only·멱등이라 step 6 package.json 머지와 동급으로 안전하므로 기존 e2e 옵트인 하네스에 업그레이드 시 멱등 재실행으로 **소급 적용**(1.17.0 README의 무소급과 대비 — README는 생성형이라 소급 불가했지만 gitignore 머지는 멱등이라 안전). **검증**: §6.16 `.gitignore` 무시 체크(no-op 케이스도 두 토큰 존재로 통과) + 자동 수정 항목. **적대적 리뷰(3관점 워크플로, 확정 12건)로 4건 수용**: (a) dedup을 정확매치 → **루트 앵커 첫 세그먼트 매칭**으로 강화 — `test-results/*`·`**/test-results/` 글로브 변형을 인식해 중복 항목 추가를 막되, 중첩 경로(`foo/test-results/`)는 leak 방지 위해 미일치(누락보다 무해한 중복 선호), 음수패턴(`!`) 존중 (b) §5.17 문구 정확화(`playwright-report/`는 기본 list/github 리포터에선 미생성, html/show-report 시 생성 — 선제 무시는 무해) (c) cross-ref §10.3→§10.1 #33 정정 (d) U1 재감지 출력목록에 머지 명시. 미반영 8건은 무해·기존 의도·범위 밖. **실측 12 시나리오**(기존 6: 부재→생성·멱등스킵·append·no-op·부분추가·무개행 + 신규 6 글로브: `test-results/*` 인식·이중 글로브 no-op·`**/` any-depth·중첩경로 미일치·음수패턴·유사명 미일치) 전부 설계대로 + 골든 픽스처(e2e·structural) 회귀 통과. 신규 프로필 필드·플레이스홀더·파일 0건, 옵트인(`e2e.enabled` 게이트). MINOR, 하위 호환. 이슈 #13 종결.
...
### 1.18.0 (마찰 자동 기록 — 저비용 JSONL 싱크, 이슈 #9, TODO-84)
- **1.18.0** (2026-06-16) — 생성 하네스의 마찰 피드백 루프 dead-letter(이슈 #9, TODO-84)를 마감. `docs/HARNESS_FRICTION.md`는 "자동 기록한다"고 명시하지만 실제 메커니즘이 없어 산문 지시에 의존했고, 마크다운 테이블 행 삽입(읽기→탐색→삽입→재작성)이 무거워 부하 시 건너뛰어져 로그가 항상 빈 테이블 → harness-feedback 분석 데이터 0건(haja-web-fe 1개월 실사용 실측). **해결**: 마찰 기록을 산문 의존 최소화한 **저비용 단일 append**로 전환 — `templates/rules/session-routine.md`의 마찰 기록을 `.harness-friction.jsonl`(append-only, 프로젝트 루트, git 커밋) 1줄 append로 교체. 한 줄=1 이벤트, 필드 `ts`(ISO8601 UTC)·`session`(세션 고유 ID)·`event`(유형 enum)·`severity`(low/medium/high/critical)·`feature`(ID 또는 `""`)·`detail`(소독된 원인 ≤50자). 세션 시작에 `SESSION_ID`를 `{ISO 시각}-{4자 난수}` 형식(예 `2026-06-16T09-12-03Z-a3f9`)으로 1회 생성해 `claude-progress.txt`에 `SESSION_ID: <값>`으로 기록(같은 날 복수 세션 구분). detail 소독 규칙(큰따옴표→작은따옴표·줄바꿈/CR 제거·백슬래시 제거·≤50자)을 session-routine.md에 명시. `HARNESS_FRICTION.md`는 로그 테이블·자동 렌더 개념을 제거하고 이벤트/심각도 참조표·이슈 보고 안내·jsonl 포인터만 남긴 **정적 참조 문서로 격하**(manifest category `data`→`managed`). harness-scaffold가 빈 `.harness-friction.jsonl`을 생성하고 manifest에 **category `data`로 등록**(템플릿 해시 드리프트 검사 제외). harness-feedback은 입력을 jsonl로 바꾸고 **관용 파싱**(JSON 실패 줄 스킵·스킵 수 보고)으로 깨진 줄 한 개가 전체 분석을 죽이지 않게 한다. 프로필 신규 필드 없음(always-on), 프로필 스키마 version 1.17.0→1.18.0. **설계 결정(옵션 i — Stop hook 없음)**: 멀티모델 자문(codex 정확성/gemini 단순화, `.claude/artifacts/consult/` 2026-06-16)을 종합 — 두 모델 합의(JSONL 싱크·always-on·`echo`로 LLM 직접 직렬화는 위험)는 채택, 충돌(codex "훅·렌더러 유지+하드닝" vs gemini "훅·렌더러·node 제거")은 스택 비종속 원칙과 measure-first 자세에 따라 **gemini 단순화를 주축으로 채택하고 codex의 견고성 디테일(깨진 줄 격리·고유 세션 ID·detail 소독)을 흡수**한다. Stop hook을 두지 않는 이유: ① 훅 발화 비보장(크래시/강제종료)이라 안전망 이상이 못 됨 ② 렌더러 폐기 시 훅에 남는 일은 false-positive 위험이 있는 휴리스틱 백필뿐 ③ 훅은 첫 `.claude/settings.json`+merge upsert+node-on-by-default+업그레이드 마이그레이션을 끌고옴 ④ 주 메커니즘(싼 append)이 실제 부하를 진다 — 측정 후 누락이 확인되면 후속 증분에서 추가. MINOR(새 managed 데이터 파일 + 생성/소비 경로 변경, 하위 호환), 업그레이드 마이그레이션 불필요(글로벌 컴패니언 harness-feedback은 install.sh 배포, 기존 하네스는 재렌더+빈 jsonl 생성으로 자기 치유). 설계 정본: .tracking/specs/2026-06-16-friction-auto-logging-design.md

### 1.17.0 (E2E 모듈 마감 — 이슈 #12 증분 4, TODO-97)
- **1.17.0** (2026-06-16) — 1.11.0~1.16.0에 걸친 E2E 옵트인 모듈을 마감. 4개 작업 + 사용자 확정 결정(D1=pre-seed·D2=e2e/README.md managed·D3=cascade·D4=단일). **A** §12.6.1 파일-템플릿 매핑에 e2e managed 3파일(playwright.config.ts·e2e/tsconfig.json·e2e/README.md) 편입 — 증분 2b가 이월한 정렬, deferral 주석 제거. **B** 프론트엔드 프리셋 3종에 `e2e:{enabled:true}` 권장 기본(pre-seed) — §4.2 옵트인 질문은 유지하되 기본 답을 "예"로 제시(거부=생략, 옵트인 계약 불변), express-api 비대상, playwrightVersion/prePush/mcp 비시드. **C** U1 base-E2E 재감지 불릿(비-e2e 프론트엔드 하네스 업그레이드 시 옵트인 제안) + pre-push cascade(신규셋업 §4.2 순서 미러). **D** 사람 개발자용 `templates/e2e/README.md`(managed, 정적, 플레이스홀더 0) 신설 — 8절(무엇·레이아웃·fixtures·셀렉터/태그·시각/레이아웃 회귀·실행·조건부 MCP/pre-push), 에이전트-규칙 정본 참조(비복제). **설계 결정(발견)**: §12.6 자동 감지는 manifest 기록 파일의 **업데이트 전용**(신규 파일 생성 불가, 그건 `[new]` 마이그레이션 영역) — 따라서 README는 신규 셋업·U1 재감지 옵트인 경로로만 생성되고 기존 e2e 하네스엔 소급 생성하지 않는다(비침습·옵트인 보존, 무마이그레이션 트레이드오프를 §10.3 1.17.0 노트에 파일별로 정직 명시). config 2종은 기존 e2e 하네스 manifest에 이미 있어 §12.6.1 편입으로 자동 감지를 새로 받음. 신규 플레이스홀더 0(31 불변), Public API 4계약 무변경(프리셋 스키마에 선택 e2e 필드 추가는 후방호환), 마이그레이션 불필요. MINOR. 설계 정본: docs/superpowers/specs/2026-06-16-e2e-incr4-design.md, 계획: docs/superpowers/plans/2026-06-16-e2e-incr4.md
