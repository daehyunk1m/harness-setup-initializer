...
- [ ] `docs/product-specs/_template.md` — PRD 작성 템플릿 (managed substrate, 부재=구조 실패)
  ...
  > 작성된 PRD의 **내용 위생**(필수 `Edge Cases` 섹션이 비어있지 않음 — 헤딩/주석/공백/단독 placeholder만이면 경고)도 harness-check ⑩이 검사한다 — **경고 전용(exit 0)**, 앵커 부재 PRD는 침묵.
...
- [ ] 인프라/설정 TDD 우회 방지: `category: infra`/`config` 트랙이 유닛 RED→GREEN을 통합 검증(빌드+실동작)으로 *대체*하되, 남용 방지 게이트(사전 선언·부정 테스트·모호 시 TDD·재분류 금지)로 전체 TDD 임의 우회를 막는가 (session-routine § 인프라/설정 트랙)
...
> **의존성 미설치 — 판정 보류**: `node_modules`가 설치되지 않은 상태(갓 클론한 프로젝트, 셋업 직후)에서는
> 품질 항목(④ 아키텍처 검사·⑤ 통합 검증)이 도구 부재(`vitest: command not found` 등)로 실행되지 못한다.
> 이는 하네스 결함이 아니라 `npm install` 1회로 자가 해소되는 **전이적 상태**이므로, harness:check은 ④⑤를
> *"의존성 미설치로 보류"*로 표기하고 구조 항목(①②③)이 정상이면 **"의존성 미설치 (구조 정상)"** 로 분기한다
> (exit 0 — 품질 실패가 아님). 하네스는 의존성을 자동 설치하지 않는다(절대 규칙). install 후 재실행하면 표준/MVH로 판정된다.
...
> **node_modules 전제**: 4·5번(아키텍처 검사·통합 검증)은 설치된 의존성이 필요하다. `node_modules` 부재 시
> harness:check은 이 두 항목을 "의존성 미설치로 보류"로 표기하고 구조가 정상이면 exit 0로 분기한다
> (품질 실패가 아니라 전이적 상태 — `npm install` 후 재실행하면 표준 판정). 자동 설치는 하지 않는다.

> 생성된 하네스에서는 위 6개 명령이 `npm run harness:check` 하나로 통합되어 있다
> (`scripts/harness-check.sh` — tsconfig paths(⑦)·E2E 스캐폴드 구조(⑧)·pre-push(⑨)·PRD 위생(⑩: 마커+내용) 포함).
