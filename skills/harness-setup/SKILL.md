...
| exists  | exists   | "이미 하네스가 셋업되어 있습니다." 안내 → 업그레이드(§ 12) 또는 재생성 옵션 제시 |
...
Step 0: 모드 판별
  ...
  ├── 하네스 파일 있음 + .harness-manifest.json 없음 → Bootstrap (§ 12.4)
  └── 하네스 파일 있음 + .harness-manifest.json 있음 → Upgrade 분석 (§ 12.3 U1~U2)
...
[Upgrade 모드]
Phase U1~U2 (§ 12.3 참조) → .harness-profile.json 저장 → Skill 도구로 /harness-scaffold 자동 실행
```
...
- 프론트엔드 신호(테스트 프레임워크가 playwright이거나, stack에 UI 프레임워크 — react/react-dom/vue/svelte/solid-js/@angular/core — 감지)가 있으면 § 4.2의 E2E 계층 질문(옵트인)을 트리거한다. 백엔드 전용(express 등)은 트리거하지 않는다. 같은 프론트엔드 신호는 § 4.2의 브라우저 MCP 진단 질문(옵트인)도 트리거한다 — E2E 스펙 옵트인과 **독립**.
...
연계 가능한 외부 보조 스킬/도구의 설치 여부를 확인한다. **미감지 시 관련 질문을 생략한다** (사용자가 모르는 도구를 노출하지 않음). 통합 메커니즘 규약: `references/integrations/_protocol.md`. 등록된 통합마다 아래 감지를 수행하고, 감지된 것만 § 4.2 질문을 트리거한다.

**superpowers** (외부 플러그인 — 매핑 정본: `references/integrations/superpowers-mapping.md`):
```bash
# 1순위: 플러그인 설치 (공식 마켓플레이스)
cat ~/.claude/plugins/installed_plugins.json 2>/dev/null | grep -o '"superpowers@[^"]*"' | head -1
# 폴백: 수동 스킬 설치
ls -d ~/.claude/skills/superpowers* 2>/dev/null | head -1
```
...
#### 3.2 매칭 로직
1. package.json의 dependencies에서 각 프리셋의 `detection.required` 키를 대조한다 (`detection.exclude`의 패키지가 존재하면 해당 프리셋 제외)
...
- (domain-based로 분류된 경우) "도메인 간 공유 코드는 어느 폴더에 두나요? (예: shared/, common/)"
...
- 매칭된 프리셋이 `e2e.enabled` **권장 기본**을 제공한 경우(프론트엔드 프리셋), 이 질문의 기본 답을 "예"로 제시한다. 단 사용자가 거부/무응답이면 동일하게 필드를 생략한다 (프리셋은 답을 강제하지 않는다 — 옵트인 계약 불변)
...
- 동의하면 프로필에 `integrations.superpowers`를 기록하고 (linkedSkills 기본 3종), 거부하면 해당 통합 필드를 생략한다 (연계 없음)
- 연계/제외 분류 정본: `references/integrations/superpowers-mapping.md`

**multi-model-consult 연계 관련** (Step 1.6에서 multiModelConsult가 감지된 경우에만, 우선순위 5 — 옵트인):
- "멀티모델 자문 스킬이 감지되었습니다 ({codex/gemini}). AGENTS.md에 연계 가이드를 삽입할까요? 복잡한 설계 결정·아키텍처 트레이드오프를 codex·gemini에 교차 자문하고 Claude가 합성합니다. TDD·리뷰·검증은 본 하네스 워크플로를 사용합니다."
- 동의하면 프로필에 `integrations.multiModelConsult`를 기록하고, 거부하면 해당 통합 필드를 생략한다
- 연계 정본: `references/integrations/multi-model-consult-mapping.md`

**자동 커밋 관련** (우선순위 5 — 옵트인, 묻지 않으면 기본 off):
- "TDD 사이클/기능 완료 시 커밋을 자동화할까요? confirm 모드는 커밋 메시지와 변경 요약을 보여주고 승인받은 뒤 commit+push 합니다 (auto는 승인 없이, off는 제안만 — 기본). **auto를 선택해도 위험 작업(push --force·reset --hard·대규모 변경·의존성)은 안전 규칙에 의해 항상 제안만 합니다 — 자동화 대상은 정상 TDD 커밋뿐입니다.**"
- 동의하면 프로필에 `autoCommit`(mode, pushAfterCommit)을 기록하고, 거부/무응답이면 생략한다 (mode off)
...
- ESLint 보조 규칙: 옵트인 여부, 설정 파일 경로/형식(flat/legacy), maxLines
- 외부 통합 (integrations.<name>): 옵트인 여부, source/version/installPath, 통합별 필드(superpowers linkedSkills 등)
- 자동 커밋 (autoCommit): 옵트인 시 mode(confirm/auto/off), pushAfterCommit
```
...
| ESLint 보조 규칙 | 미적용 (`eslintAssist` 필드 생략) | 기존 설정 파일 수정은 명시적 동의가 필요 |
| 공유 모듈 디렉토리 (domain-based) | `shared/`, `common/`, `core/` 중 실제 존재하는 폴더, 없으면 `["shared"]` | 디렉토리 탐색 |
| 외부 통합 (integrations) | 미연계 (`integrations` 필드 생략) | 외부 의존 연계는 명시적 동의가 필요. 미감지 시 질문 자체를 생략 |
| 자동 커밋 (autoCommit) | 미적용 (`mode: "off"` — 제안만) | 자동 git 실행은 명시적 동의가 필요. 기본은 기존 "제안만" 동작 |
...
6. .claude/rules/git-workflow.md (Git 규칙)
7. agents/*.md (7개 TDD subagent 정의)
8. claude-progress.txt
9. feature_list.json
10. init.sh
...
### 수정 예정 파일
- package.json: scripts에 lint:arch, validate, doc:check, harness:check 추가
- {ESLint 설정 파일}: 보조 규칙 추가 — eslintAssist 옵트인 시에만 이 행을 표시한다
- .claude/rules/git-workflow.md: 자동 커밋 정책 mode={autoCommit.mode} — autoCommit 옵트인 시에만 이 행을 표시한다
...
> **두 승인의 구분**: 여기(Step 5)서 승인하는 것은 *프로필*(스택·아키텍처 분류·생성 계획)이다. 생성된 문서(AGENTS.md/ARCHITECTURE.md)의 *의미 정확성*은 scaffold **Phase 4의 "아키텍처 정확성 확인" 게이트**(harness-scaffold/SKILL.md § 7)에서 별도로 확인받는다 — 문서는 이 프로필 승인 이후 생성되기 때문이다.
...
| `autoCommit` (선택) | Step 4 옵트인 | 생략 시 `mode: "off"`(제안만 — 기존 동작). `mode`는 `off`/`confirm`(승인 후 commit+push)/`auto`(자동). `pushAfterCommit`은 커밋 후 push 여부(기본 true). git-workflow.md `{{AUTO_COMMIT_MODE}}`·`{{AUTO_COMMIT_PUSH}}`의 원천. 프리셋 비대상 |
...
├── react-router-fsd.json  # React + React Router v7 — FSD
...
**e2e 참고** (선택): 프론트엔드 프리셋은 브라우저 E2E 계층의 **권장 기본**(`{ "enabled": true }`)을 제공할 수 있다. 이는 § 4.2 E2E 옵트인 질문의 *기본 답을 "예"로* 제시한다는 의미이며, 질문 자체는 유지된다 — 거부/무응답이면 `e2e` 필드를 생략하고 모듈을 생성하지 않는다(옵트인 계약 불변). `playwrightVersion`은 프리셋에 하드핀하지 않는다(스킬보다 빨리 stale) — § 4.2가 현재 stable로 채운다. `prePush`/`mcp` 기본값은 프리셋에서 시드하지 않는다(각각 git/MCP를 전제하는 독립 2차 옵트인). 백엔드 프리셋(express-api 등)에는 두지 않는다(§ 1.4에서 트리거 자체가 안 됨).
...
3.3. `detection.exclude`가 있으면, 그중 한 패키지라도 dependencies/devDependencies에 존재할 경우 해당 프리셋을 후보에서 제외한다 (더 구체적인 스택과의 오매칭 방지 — 예: react-vite는 next/react-router 존재 시 제외)
...
4.5. `detection.exclude`로 더 구체적인 스택과의 오매칭을 방지한다 (선택 — 예: react-vite는 `next`/`react-router` 존재 시 제외. required가 범용 패키지일 때 필수적으로 검토)
4.6. (프론트엔드 한정, 선택) `e2e: { "enabled": true }`로 브라우저 E2E 계층의 **권장 기본**을 둔다 — § 4.2 옵트인 질문은 유지되고 거부/무응답이면 생략된다(옵트인 계약 불변). `playwrightVersion`은 하드핀하지 않는다(§ 4.2가 채움). 백엔드 프리셋엔 두지 않는다.
...
> **구현 완료**: 2-스킬 분리 (분석 + 스캐폴딩), 프로필 저장, 업그레이드 시스템, Cleanup 스킬 (`companion-skills/harness-cleanup/` — 운영 사이클 실행 주체), 피드백 분석 스킬
...
- `references/harness-checklist.md` — 하네스 구성 체크리스트 (생성 하네스의 판정 기준, §8 자가진단)
- `references/integrations/_protocol.md` — 외부 통합 규약 (integrations.<name> 메커니즘 정본)
- `references/integrations/superpowers-mapping.md` — superpowers 연계/제외 분류 정본 (14종 전수)
...
- `references/root-placement-contract.md` — 루트 배치 계약 (생성 산출물의 루트 고정/이동 가능 등급 + 재배치 결정·트리거)
...
| 결정 | 근거 |
|------|------|
| **전체 프로필 저장** | 플레이스홀더가 31개이고 소스가 분산되어 있어, 업그레이드 시 재스캔/재문답 없이 managed 파일을 재생성하려면 전체 프로필이 필요하다 |
...
| 22-b | `.harness-friction.jsonl` | data | 마찰 이벤트 자동 기록 싱크(append-only JSONL). 템플릿 해시 드리프트 검사 제외 — feature_list와 동일 취급 |
...
Phase U1: 분석
  ...
  ├── 외부 통합 재감지 (Step 1.6과 동일 — 등록된 통합 전부):
  │     - 새로 감지된 통합 + manifest.profile.integrations에 없음 → U2에서 통합 추가 제안 (옵트인)
  │     - 기존 통합 있음 → 실존 재검증(superpowers는 linkedSkills, consult는 CLI), 사용자가 제거를 요청하면 fileActions로 보조 스킬 섹션에서 해당 항목 제거
...
Step 5: manifest 쓰기 → 정상 업그레이드 플로우 (§ 12.3) 진입
```

#### 설계 결정

- **1.0.0 고정**: 현재 상태를 semver 1.0.0으로 선언한다. pre-semver 시절의 모든 기존 프로젝트를 1.0.0으로 편입한다. 상세: `references/versioning-policy.md`
- **해시 = 현재 상태**: 부트스트랩 시 기록되는 templateHash는 "현재 파일 상태"이다. 따라서 첫 업그레이드에서 managed 파일의 해시가 불일치할 수 있고, 이 경우 § 12.2의 3가지 선택지를 제시한다.
