...
- docs/TECH_DEBT.md의 "자동 검사 승격 대기 큐" (있을 경우)
- (인프라/설정 트랙이면) 해당 feature가 인프라로 분류된 근거 — 부정 테스트 판정 포함
...
### 4. 반복 지적 감지
- 이번 발견 사항이 승격 대기 큐의 기존 항목과 같은 유형의 위반이면, 해당 발견을 **자동 검사 승격 후보**로 표시한다
- 큐에 없는 새 유형이라도 자동 검사(ESLint 규칙, structural-test 확장, 테스트)로 잡을 수 있는 위반이면 승격 후보로 표시한다

### 5. 인프라/설정 트랙 분류 감사 (조건부)

이번 feature가 **인프라/설정 트랙**(category가 `infra`/`config`, TDD STATE `track: infra`)으로 처리된 경우에만 수행한다. 이 트랙은 유닛 RED→GREEN을 스킵하므로 Reviewer가 **분류 타당성의 유일한 독립 검증자**다.

- **부정 테스트 재검**: 변경이 추가한 동작에 대해 *의미 있는 실패 테스트(유닛/E2E)를 작성할 수 있었는가?* 작성 가능했다면(테스트 가능한 변환·매핑·계산·도메인 로직이 포함됐다면) **분류가 틀린 것**이다 → **NEEDS_FIX**로 판정하고 "인프라가 아니라 TDD 대상 — 해당 단위를 분리해 RED→GREEN으로 재구현" 사유를 명시한다.
- **보안 표면 누락 점검**: 변경이 보안 표면(.env·secrets·auth/세션/토큰·provider 배선·CORS·쿠키)에 닿았는데 Security Reviewer가 호출되지 않았으면 그 누락을 지적한다.
...
### 자동 검사 승격 후보
{반복 지적 또는 자동 검사로 잡을 수 있는 위반이 있는 경우}
- 규칙: {위반된 규칙 한 줄} / 근거: {이번 발견 + 큐의 기존 횟수} / 제안 검사: {ESLint 규칙 | structural-test 확장 | 테스트}
```
...
- TECH_DEBT.md에 기록할 가치가 있는 발견은 별도로 표시한다 — 직접 기록하지 않고 표시만 한다 (기록은 오케스트레이터가 수행)
