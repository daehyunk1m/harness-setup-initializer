...
| 월간 | AGENTS.md/ARCHITECTURE.md 전면 검토 + feature_list passes 재검증 + `npm run harness:check` + 미보고 마찰 피드백 분석(보조) |

주간/격주/월간 작업은 harness-cleanup 스킬로 실행할 수 있다 (harness-setup 플러그인에 번들 — "하네스 정리" 요청).
...
## 하네스 이슈 보고
- 하네스 관련 문제 발생 시 `.harness-friction.jsonl`에 JSON 한 줄을 append한다 (이벤트 스키마: `ts/session/event/severity/feature/detail`)
...
- **category 값**: 사용자 기능은 도메인 카테고리(`core`, 화면/모듈명 등). 보안 인접 기능은 `tdd.securityCategories`(auth/security/api/payment 등)에 맞춰 분류하면 셋업 후 에이전트가 보안 리뷰를 트리거한다. **배선/설정/도구 작업은 `infra` 또는 `config`** — 이 카테고리는 셋업 후 에이전트가 session-routine.md § 인프라/설정 트랙(유닛 RED→GREEN을 통합 검증으로 대체, 남용 방지 게이트 포함)으로 처리한다. 기존 프로젝트 추론 시 빌드/도구 설정·provider 배선·DI 셋업 같은 비-도메인 모듈을 발견하면 `infra`로 분류한다(단, 단순 라우트/기능 모듈은 일반 도메인 카테고리 유지 — infra 남발 금지)
  - **다단계 인프라는 Phase별로 분해**: 여러 세션에 걸친 다단계 인프라 작업(예: 백엔드 연동 7-Phase)은 하나의 큰 항목이 아니라 Phase별 개별 infra 항목으로 분해한다(`F-infra-0`, `F-infra-1` …). 진행 상태는 각 항목의 `passes`/`priority`/`last_session`으로 자연히 표현되어 별도 진행 추적 블록이 필요 없다(session-routine.md § 다단계·다세션 인프라 작업)
...
1. **실존 검증**:
   ...
   - multiModelConsult류(번들·CLI): 스킬은 플러그인 번들이라 항상 존재 → `installPath`(null)는 검사하지 않고 **자문 CLI(codex/gemini) 1개 이상만** 확인. CLI 0개면 통합 스킵 + 안내
...
7. 정기 운영은 CLAUDE.md의 "운영 사이클" 섹션을 따르세요 (일간 validate / 주간 품질 / 격주 부채 / 월간 전면 검토 + `npm run harness:check` + 미보고 마찰 피드백 분석(보조))
...
- 하네스 정리 → "하네스 정리" / "월간 점검" (상세: CLAUDE.md 운영 사이클 — 플러그인 번들 스킬)
...
- **멀티모델 자문 줄**: `integrations.multiModelConsult.enabled === true`이고 § 5.16 실존 검증(번들이라 스킬 존재는 보장 + codex/gemini CLI ≥ 1)을 통과한 경우에만 렌더. CLI가 없으면 줄 자체를 생략한다.
...
- **검증 게이트 · 자가진단 · 품질·부채 추적 · 마찰 자동 기록 · 의도 적재 · PRD 명세 줄**: 항상 생성되는 산출물이므로 무조건 렌더. PRD 명세 줄은 substrate(관례·템플릿)가 always-on 생성되므로 표시하되, **문구는 "작성 관례·템플릿 제공"에 한정**한다 — 의도↔PRD 커버리지 derive는 **별도 "의도 증류" 줄이 광고**하므로(2b-2 출시) PRD 명세 줄은 작성 관례에 집중한다(능력 중복 광고 회피).
...
- **수정 요청 / 불일치** → 무엇이 틀렸는지 듣고, ARCHITECTURE.md를 직접 수정하거나 `/harness-setup`을 재실행하여 분류를 교정하도록 안내한다. `.harness-friction.jsonl`에 `setup-mismatch` 또는 `rule-conflict` 이벤트를 JSON 한 줄로 append하도록 권유한다(마찰 루프 입력). `semanticApprovalAt`는 `null`로 둔다
...
#### M-1.4.0 이후 (1.5.0 ~ 1.6.4): 마이그레이션 불필요
